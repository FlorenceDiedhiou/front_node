...
const Inscription = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await fetchApi('/api/auth/inscription', {
        method: 'POST',
      ...
      if (response.ok) {
        toast.success('Inscription réussie. Vous pouvez vous connecter.', { id: toastId })
        navigate('/connexion')
      } else {
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-violet-100/50 border border-slate-100 p-8 sm:p-10 animate-fade-in-up">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="prenom" className="block text-sm font-medium text-slate-700 mb-1.5">Prénom</label>
                <div className="relative">
                  ...
                  <input id="prenom" type="text" placeholder="Florence" value={prenom} onChange={(e) => setPrenom(e.target.value)} className={inputClass} />
                </div>
              </div>

              <div>
                <label htmlFor="nom" className="block text-sm font-medium text-slate-700 mb-1.5">Nom</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                    <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  ...
                  <input id="nom" type="text" placeholder="Diedhiou" value={nom} onChange={(e) => setNom(e.target.value)} className={inputClass} />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="email" type="email" placeholder="exemple@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} />
              </div>
            </div>
